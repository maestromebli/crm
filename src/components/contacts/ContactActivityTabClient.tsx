...
export function ContactActivityTabClient({ contactId }: Props) {
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 max-w-xl text-sm text-slate-600">
                Події, повʼязані з цим контактом у журналі CRM (оновлення профілю,
                файли тощо).
              </p>
            </div>
          </div>
        ...
        {err ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/80 px-6 py-12 text-center">
            ...
            <p className="text-sm font-medium text-slate-800">
              {items.length === 0
                ? "Поки немає записів для контакту"
                : "Немає подій у цьому фільтрі"}
            </p>
            <p className="mt-1 max-w-sm text-xs text-slate-600">
              {items.length === 0
                ? "Журнал активності для контакта заповнюється при змінах профілю та повʼязаних діях."
          ...
          <div className="relative">
            <div
              className="pointer-events-none absolute bottom-3 left-[19px] top-3 w-px bg-gradient-to-b from-slate-200 via-slate-200 to-transparent md:left-[21px]"
              aria-hidden
            />
                ...
                <div key={dayIso}>
                  ...
                  <ul className="space-y-0">
                    {dayItems.map((it) => {
                      ...
                      const abs = format(new Date(it.createdAt), "HH:mm", {
                        locale: uk,
                      });
                      return (
                        <li
