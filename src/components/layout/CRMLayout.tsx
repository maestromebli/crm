...
/**
 * Обгортка Dynamic Layer:
 * зберігає основний контент і монтує розумну панель як опціональну праву колонку.
 */
export function CRMLayout({ main, smartPanel, className }: Props) {
  return (
    <div className={cn("grid grid-cols-1 gap-3 xl:grid-cols-[2fr_1fr] xl:items-start", className)}>
      ...
      <aside className="enver-panel enver-panel--interactive rounded-2xl p-2">{smartPanel}</aside>
    </div>
  );
}
