...
const neutralPill =
  "border-slate-200 bg-[var(--enver-card)] text-slate-800 hover:bg-[var(--enver-hover)]";
...
export function DealWorkspacePrimaryActions({
  ...
  const [statusOverride, setStatusOverride] = useState<typeof data.deal.status | null>(null);
  const localStatus = statusOverride ?? data.deal.status;
  ...
  const patchStatus = useCallback(
    async (status: "WON" | "LOST") => {
      if (
        !confirm(
          status === "WON"
            ? "Позначити замовлення як виграну?"
            : "Позначити замовлення як втрачену?",
        )
      ) {
        return;
      }
