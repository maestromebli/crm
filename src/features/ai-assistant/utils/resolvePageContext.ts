...
function hintForHeadManager(ctx: ResolvedPageContext): ContextHint {
  if (ctx.kind === "leads_list" || ctx.kind === "lead_detail") {
    return {
      ...
      suggestedNextStep:
        "Розподіліть навантаження або закріпіть наступний контакт за відповідальними менеджерами.",
      hasSuggestion: true,
      tone: "attention",
    };
  }
...
function hintForSales(ctx: ResolvedPageContext): ContextHint {
  ...
  if (ctx.kind === "deal_detail" || ctx.kind === "deal_workspace") {
    return {
      ...
      suggestedNextStep:
        "Перевірте активну версію розрахунку та надішліть наступний контакт, якщо КП без відповіді.",
      hasSuggestion: true,
      tone: "neutral",
    };
  }
