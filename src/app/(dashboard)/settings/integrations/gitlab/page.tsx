...
export default async function SettingsGitLabPage() {
  ...
  return (
    <SettingsShell
      title="GitLab"
      description="Підключення до GitLab (self-managed або GitLab.com) через REST API: перевірка токена, подальші сценарії — issues, pipelines, посилання на репозиторії."
    >
      ...
      >
        <ul className="list-inside list-disc space-y-1 text-[11px] text-slate-600">
          <li>
            <code className="rounded bg-slate-100 px-1">GITLAB_BASE_URL</code> — базова URL інстансу,
            наприклад <code className="rounded bg-slate-100 px-1">https://gitlab.com</code> або ваш
            корпоративний хост.
          </li>
          <li>
            <code className="rounded bg-slate-100 px-1">GITLAB_TOKEN</code> — personal access token
            або project/group token з мінімальними scope: <code className="rounded bg-slate-100 px-1">read_api</code>{" "}
      ...
      <SettingsCard
        ...
        description="Після успішної перевірки можна додати webhook-и, створення issues з угод, статуси pipeline по проєкту."
      >
        <p className="text-[11px] text-slate-600">
          Endpoint для тесту: <code className="rounded bg-slate-100 px-1">GET /api/integrations/gitlab</code>{" "}
