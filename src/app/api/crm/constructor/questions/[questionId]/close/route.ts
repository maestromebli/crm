...
import { closeConstructorQuestion } from "@/features/constructor-hub/server/constructor-question.service";
...
export async function POST(request: Request, context: Ctx) {
  ...
  try {
    const data = await closeConstructorQuestion({
      questionId,
      actorUserId: user.id,
    });
    return jsonSuccess(requestId, { data });
  } catch (e) {
    return jsonError(requestId, e instanceof Error ? e.message : "Ошибка закрытия вопроса", 400);
  }
}
