...
import { canReturnConstructorVersion } from "@/features/constructor-hub/server/constructor-rbac";
...
export async function POST(request: Request, context: Ctx) {
  ...
  if (!canReturnConstructorVersion(user, scope)) return jsonError(requestId, "Недостаточно прав", 403);
  ...
  try {
    ...
    if (!payload.comment?.trim()) return jsonError(requestId, "Причина возврата обязательна", 400);
    const data = await reviewConstructorVersion({
      ...
      payload: {
        ...
        remarksJson: payload.remarksJson ?? null,
      },
    });
    return jsonSuccess(requestId, { data });
  } catch (e) {
    return jsonError(requestId, e instanceof Error ? e.message : "Ошибка return-for-revision", 400);
  }
}
