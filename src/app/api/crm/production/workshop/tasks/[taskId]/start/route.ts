...
export async function POST(_request: Request, context: Ctx) {
  ...
  try {
    ...
    if (!canManage && task.assigneeUserId !== user.id) {
      return NextResponse.json({ error: "Оператор може запускати лише призначені задачі" }, { status: 403 });
    }
    await startWorkshopTask({
      taskId,
      actorName: user.id,
    });
    ...
    return NextResponse.json({ error: "Не вдалося запустити задачу" }, { status: 500 });
  }
}
