...
const ROOM_UPLOAD_CATEGORIES: AttachmentCategory[] = [
  "DRAWING",
  "MEASUREMENT_SHEET",
  "OBJECT_PHOTO",
  "REFERENCE",
...
export async function POST(req: Request, ctx: Ctx) {
  ...
  if (!ROOM_UPLOAD_CATEGORIES.includes(category)) {
    return NextResponse.json(
      { error: "Ця категорія недоступна для завантаження з цієї сторінки" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Завантаження ще не доступне" },
        { status: 403 },
      );
    }
