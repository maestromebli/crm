-- Таблиця віх оплати по угоді; має існувати до FK з FinanceInvoice.dealPaymentMilestoneId.
...
-- Якщо FinanceInvoice вже є, але FK на віху не додався через відсутність таблиці.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'FinanceInvoice')
     AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FinanceInvoice_dealPaymentMilestoneId_fkey') THEN
